...
- Bets lock the odds at placement; later odds moves never change an open bet.
  ...
  **except the in-play goal Asian Handicap (`ah_goals`)**, which settles on the
  **remainder of the match** (goals before the bet ignored). Bet365 quotes that
  live line relative to the current score, so settlement must subtract the
  placement score (`bets.live_home_score`/`live_away_score`; `settleSelection`'s
  `baseline` arg). This is the *only* market where in-play and pre-match
  settlement differ — corner AH, all totals, 1X2, BTTS and correct score are
  quoted cumulatively and settle full-match, so `settleSelection` stays
  price-source- and timing-independent for them.
- **Knockout extra time is phase-aware.** A knockout (`group_name` null) settles
  on the **90' regulation result** for every pre-match and regulation-phase
  in-play bet — extra time and penalties never count. When the feed shows extra
  time (ESPN `status.period` ≥ 3, captured in `live_state.period`), the frozen
  regulation goal score (`live_state.reg_home_score`/`reg_away_score`, frozen
  while period ≤ 2) is final, and `maybeSettleRegulationGoalBets` pays those bets
  immediately at full time (the match stays `scheduled`). In-play then **re-opens
  for goal markets only** (goal AH + goal totals — `liveMarketsForMatch`'s
  `etGoalsOnly`); penalties (period ≥ 5) are closed. Bets struck in ET are tagged
  `bets.live_period` ≥ 3 (`isEtPhaseBet`) and settle on the **end-of-ET goal
  score** (ESPN's final goal total, penalties excluded) at completion —
  goal-total cumulative, goal-AH on the placement baseline. `settleMatch` is
  phase-aware: `getFrozenRegulationScore` (gated on period ≥ 3 so a stale freeze
  never overrides a regulation/group final) grades regulation-phase bets on 90',
  ET-phase bets on the end-of-ET score. Corner/card bets stay full-match (ESPN
  only publishes full-match box scores). `getKnockoutRegulationResult` fails
  closed unless the frozen score is a level draw from a fresh feed.
