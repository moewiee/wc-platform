...
| `espn.ts` | Keyless score source: ESPN public scoreboard JSON (`site.api.espn.com/.../soccer/fifa.world/scoreboard?dates=YYYYMMDD-YYYYMMDD`) gives finals + corners (`wonCorners`); `fetchEspnCards` reads total yellow+red from the per-match `summary` box score. No odds. ESPN team names match the seeds via `normTeam` — verified for all 72 group fixtures. |
