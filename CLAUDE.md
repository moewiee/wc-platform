...
| `matches.ts` | Odds + score sync. Odds refresh throttled to 30 min, scores to 10 min (throttle stamps in `meta` table). Scores come from The Odds API when `ODDS_API_KEY` is set, otherwise from ESPN (keyless) — either way finished matches auto-settle, then a best-effort completion pass fills corners/cards from ESPN box scores so those bets settle too. Upsert matches by `api_id`, falling back to normalized team-pair — only against `scheduled` rows; voided rows release their `api_id` so rescheduled fixtures reappear. |
...
| `espn.ts` | Keyless score source: ESPN public scoreboard JSON (`site.api.espn.com/.../soccer/fifa.world/scoreboard?dates=YYYYMMDD-YYYYMMDD`) gives finals + corners (`wonCorners`); `fetchEspnCards` reads total cards (yellow = 1, red = 2) from the per-match `summary` box score. No odds. ESPN team names match the seeds via `normTeam` — verified for all 72 group fixtures. |
